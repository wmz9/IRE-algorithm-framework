...
# @MODEL_REGISTRY.register
def resnet50_nosequential(num_classes: int = 10,
             in_channels: int = 3
             ) -> ResNet:
    """ ResNet by He+16
    """
    return resnet(num_classes, 50, in_channels)
...
def get_model(model_name,num_classes):
    if model_name=='vit_t':
        ...
        model=resnet20_nosequential(num_classes=num_classes)
    elif model_name=='WRN-28-10':
        model=wrn28_10_nosequential(num_classes=num_classes)
    elif model_name=='resnet50':
        model=resnet50_nosequential(num_classes=num_classes)
    elif model_name=='resnet56':
        model=resnet56_nosequential(num_classes=num_classes)
    elif model_name=='resnet110':
        model=resnet110_nosequential(num_classes=num_classes)
    elif model_name=='resnext':
        model=resnext29_32x4d_nosequential(num_classes=num_classes)
    else: 
        raise NotImplementedError('Model has to be one of vit_t, vit_s, resnet20, resnet50, resnet56, resnet110, resnext, WRN-28-10, but is set to {}'.format(model_name))
    return model