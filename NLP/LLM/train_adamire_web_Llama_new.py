    ...
    while True:
        ...
        if iter_num > warmup_iters and iter_num % mask_interval == 1:
            ...
            for micro_step in range(gradient_accumulation_steps):
                ...
                X, Y = get_batch('train', batch_size) # more indent
                samp_dist = torch.distributions.Categorical(logits=logits) # more indent
                y_sample = samp_dist.sample() # more indent
                loss = F.cross_entropy(logits, y_sample.view(-1), ignore_index=-1) # more indent
...
# # To run this .py (ddp)
# torchrun --standalone --nproc_per_node=8 train_adamire_web_Llama_new.py --batch_size=12 --grad_micro_steps=5 --total_bs=480 --rank=0.4 --prog=2.0  --max_lr=1.5e-4 --wandb_log --wandb_run_name=moving1.5_2_04
